...
class Scanner(damaz_scan.Scanner):
    ...
    def page_scan(self, index):
        ...
        WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, ".sr-content-container")))
    ...
    def mine(self, ide):
        try:
            driver = self.driver()
            driver.get("https://careers.google.com/jobs#!t=jo&jid="+ ide)
            #print('Mine: %s' % ("https://careers.google.com/jobs#!t=jo&jid=" + ide))
            WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, '.secondary-text')))
            ...
            WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div > a.secondary-text")))
            WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "div.with-benefits > p"))) # ADDED LINE
            response = driver.page_source
            driver.close()
            driver.quit()
            soup = BeautifulSoup(response, 'lxml')
            ...
            self.dict[self.company+' '+ide]["Posting date"] = datetime.datetime.now()
            self.dict[self.company+' '+ide]["URL"] = "https://careers.google.com/jobs#!t=jo&jid=" + ide
            self.dict[self.company+' '+ide]["Basic Skills"] = '\n'.join([d.text for d in qual])
            self.dict[self.company+' '+ide]["Pref Skills"] = '\n'.join([d.text for d in qual])
        except dryscrape.mixins.WaitTimeoutError as ex:
